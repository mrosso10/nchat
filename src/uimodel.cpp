    ...
    if (!msg.fileInfo.empty())
    {
      ...
      if (fileInfo.fileStatus == FileStatusDownloaded)
      {
        ...
        if (pos != std::string::npos) {
          ...
          const std::string trueCmd = cmd + " --output_dir " + FileUtil::DirName(fileInfo.filePath) + " " + str + " 2>> /var/log/whisper/error.log >> /var/log/whisper/output.log";
          int pid = getpid();
...
bool UiModel::Impl::IsChatForceHidden(const std::string& p_ProfileId, const std::string& p_ChatId)
{
  ...
  std::string filepath_blk = FileUtil::GetApplicationDir() + std::string("/blocked_chats.conf");
  std::string filepath_allow = FileUtil::GetApplicationDir() + std::string("/allowed_chats.conf");
  ...
  return statusBroadcastHidden && (
          ...
          (
            !AppUtil::GetForceShowHiddenChats() &&
              !isStringInFile(filepath_allow, p_ChatId) &&
              (
                isStringInFile(filepath_blk, p_ChatId)
              )
          ) ||
          (
            !AppUtil::GetForceShowMutedChats() &&
